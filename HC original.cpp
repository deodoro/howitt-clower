...
#define rnd01 ((1+random(1000))/1000.0)
...
time_t begin, finish, firstbegin;
FILE *stream;
...
int Research(void) {     // Assume that we have tested U(r) first. Don't need to use m1
  for(h=0;h<=1;h++) targ[h]=random(xMax)+1;   // or m0 after that until we get to search.
  double U;
